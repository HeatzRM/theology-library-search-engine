    ...
    if len(list_of_books) == 0:
        ...
        for q in query:
            ...
            for book in split_search_books:
                final_split_search_books.update(
                    {split_search_books[book]["ID"]: split_search_books[book]}
                )
...
def get_specific_search(queries):
    ...
    try:
        ...
        print(ex)
    return query_database_by_book_id(
        SearchHandler().specific_search(queries, json_books)
    )
